...
triggers query => qr'^
    (?: [0-9 () τ π e √ x × ∙ ⋅ * + \- ÷ / \^ \$ £ € \. \, _ ! = % ]+ |
    \d+\%=?$ |
    what\s?is| calculat(e|or) | solve | math |
    times | mult | multiply | divided\s?by | plus | minus | cos | tau |
    sin | tan | cotan | log | ln | exp | tanh |
    sec | csc | squared | sqrt | mod | modulo | gross | dozen | pi |
    score){2,}$
'xi;
...
handle query => sub {
    ...
    $query =~ s/(\d+)\s+(\d+)/$1 * $2/g; # swap out spaces between numbers for multiplication
